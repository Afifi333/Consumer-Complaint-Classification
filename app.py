...
def predict_distilbert(text: str):
    ...
    try:
        clf = pipeline("text-classification", model=model_dir,
                       tokenizer=model_dir, return_all_scores=True)
        raw = clf(text[:512])[0]
        le  = bert_le if bert_le is not None else label_encoder
        if le is not None:
            result = {le.classes_[int(r["label"].split("_")[-1])].replace("_"," ").title(): r["score"]
                      for r in raw}
        else:
